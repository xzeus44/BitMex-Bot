...
namespace BitMexSampleBot
{
    public partial class Form1 : Form
    {
        ...
        private void SetBotMode()
        {
                ...
                if (rdoBuy.Checked)
                {
                if ((Candles[1].MA1 > Candles[1].MA2)) // && (Candles[2].MA1 > Candles[2].MA2) && (Candles[1].MA1 < Candles[2].MA1)) // && (Candles[2].MA1 <= Candles[2].MA2)) // Most recently closed candle crossed over up
                {
                    Mode = "Buy";
                }
            ...
            {
                if ((Candles[1].MA1 < Candles[1].MA2) && (Candles[2].MA1 < Candles[2].MA2) && (Candles[1].MA1 > Candles[2].MA1)) //&& (Candles[2].MA1 <= Candles[2].MA2)) 
                {
                    Mode = "Sell";
                }
            ...
            {
                //NEW
                if ((Candles[1].MA1 > Candles[1].MA2)) // && (Candles[2].MA1 > Candles[2].MA2) && (Candles[1].MA2 < Candles[2].MA2))   // Most recently closed candle crossed over up
                {
                ...
                else if ((Candles[1].MA1 < Candles[1].MA2)) // && (Candles[2].MA1 < Candles[2].MA2)  && (Candles[1].MA2 > Candles[2].MA2))
                {
        ...
        private void tmrAutoTradeExecution_Tick(object sender, EventArgs e)
        {
            ...
            if(chkAutoMarketTakeProfits.Checked && OpenPositions.Any() && Mode != "Sell" && Mode != "Buy") // See if we are taking profits on open positions, and have positions open and we aren't in our buy or sell periods
            {
                ...
                if (Convert.ToDouble(OpenPositions[0].UnrealisedRoePcnt * 100) >= Convert.ToDouble(nudAutoMarketTakeProfitPercent.Value))
                {
                    ...
                    // Получите наши позиции и заказы еще раз, чтобы иметь возможность обрабатывать остальную логику с новой информацией.
                    OpenPositions = bitmex.GetOpenPositions(ActiveInstrument.Symbol);
                    OpenOrders = bitmex.GetOpenOrders(ActiveInstrument.Symbol);
                }
            }
            ...
            else if(rdoSwitch.Checked)
            {
                switch(Mode)
                {
                    case "Buy":
                        if (OpenPositions.Any())
                        {
                            int PositionDifference = Convert.ToInt32(nudAutoQuantity.Value - OpenPositions[0].CurrentQty);
                                ...
                                if (OpenOrders.Any())
                                {
                                // Если у нас есть открытый ордер, отредактируйте его
                                if (OpenOrders.Any(a => a.Side == "Sell"))
                                    {
                                    // У нас все еще есть открытый ордер на продажу, отменить этот заказ, сделать новый заказ на покупку
                                    string result = bitmex.CancelAllOpenOrders(ActiveInstrument.Symbol);
                                        AutoMakeOrder("Buy", PositionDifference);
                                    }
                                    else if (OpenOrders.Any(a => a.Side == "Buy"))
                                    {
                                    // Отредактируйте наш единственный открытый порядок, код не должен позволять более 1 за раз.
                                    string result = bitmex.EditOrderPrice(OpenOrders[0].OrderId, CalculateMakerOrderPrice("Buy"));
                                    }
                                }
                                else
                                {
                                // Нет открытых ордеров, сделайте один для разницы
                    ...
                    case "CloseLongsAndWait":
                        if (OpenPositions.Any())
                        {
                            // Теперь у нас есть открытые заказы? Если это так, мы хотим убедиться, что они по правильной цене
                            if (OpenOrders.Any())
                            {
                                if (OpenOrders.Any(a => a.Side == "Buy"))
                                {
                                    // У нас все еще есть открытый заказ на покупку, отменить этот заказ, сделать новый заказ на продажу
                                    string result = bitmex.CancelAllOpenOrders(ActiveInstrument.Symbol);
                                    AutoMakeOrder("Sell", Convert.ToInt32(OpenPositions[0].CurrentQty));
                                }
                                else if (OpenOrders.Any(a => a.Side == "Sell"))
        ...
        private void btnManualSetStop_Click(object sender, EventArgs e)
        {
            ...
            if(OpenPositions.Any()) // Только установить stop , если у нас есть открытые позиции

            {
                // Теперь определите, какой stop установить
                if (OpenPositions[0].CurrentQty > 0)
                {
                    // Определите стоп-цену, х% ниже текущей цены.
                    ...
                    // Round the Stop Price down to the tick size so the price is valid Закруглите стоп-цену до размера галочки, чтобы цена была действительной
                    StopPrice = StopPrice - (StopPrice % ActiveInstrument.TickSize);
                    // Set a stop to sell Установите stop для sell
                ...
                {
                    // Determine stop price, x percent below current price.Определите стоп-цену, х% ниже текущей цены.
                    double PercentPriceDifference = Convert.ToDouble(Candles[0].Close) * (Convert.ToDouble(nudStopPercent.Value) / 100);
                    double StopPrice = Convert.ToDouble(Candles[0].Close) + PercentPriceDifference;
